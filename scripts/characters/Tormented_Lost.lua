...
local char = Isaac.GetPlayerTypeByName("Tormented Lost")
local item_id = Isaac.GetItemIdByName("Forbidden dice")
local game = Game()

function Mod:TormentedLostNoHp()
...
Mod:AddCallback(ModCallbacks.MC_POST_UPDATE, Mod.TormentedLostNoHp)

function Mod:TormentedLostInit(player)
	if player:GetPlayerType() ~= char then
		return
	end
